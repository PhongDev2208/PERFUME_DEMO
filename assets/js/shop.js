...
// DOM elements
let searchInput, productsGrid, totalProductsSpan, showingResultsSpan, totalResultsSpan;
...
function initializeElements() {
    ...
    // Filter elements
    genderFilters = document.querySelectorAll('input[id^="filter-"][id$="men"], input[id^="filter-"][id$="women"], input[id="filter-unisex"]');
    brandFilters = document.querySelectorAll('input[id^="filter-"][id*="creed"], input[id^="filter-"][id*="tom"], input[id^="filter-"][id*="chanel"], input[id^="filter-"][id*="dior"], input[id^="filter-"][id*="le-labo"], input[id^="filter-"][id*="byredo"], input[id^="filter-"][id*="maison"], input[id^="filter-"][id*="ysl"], input[id^="filter-"][id*="diptyque"], input[id^="filter-"][id*="armani"], input[id^="filter-"][id*="hermes"]');
    seasonFilters = document.querySelectorAll('input[id^="filter-"][id*="spring"], input[id^="filter-"][id*="summer"], input[id^="filter-"][id*="autumn"], input[id^="filter-"][id*="winter"]');
...
function initializeEventListeners() {
    ...
    genderFilters.forEach(filter => {
        filter.addEventListener('change', () => handleFilterChange('gender', filter.value, filter.checked));
    });
    
    brandFilters.forEach(filter => {
        filter.addEventListener('change', () => handleFilterChange('brand', filter.value, filter.checked));
    });
    
    seasonFilters.forEach(filter => {
        filter.addEventListener('change', () => handleFilterChange('season', filter.value, filter.checked));
    });
    ...
    if (showMoreBtn && moreBrands) {
        showMoreBtn.addEventListener('click', () => {
            if (moreBrands.style.display === 'none') {
                ...
                showMoreBtn.innerHTML = '<i class="fas fa-chevron-down me-1"></i>Xem thêm';
            }
        });
    }
...
function sortProducts(products, sortType) {
    switch (sortType) {
        ...
        case 'brand-asc':
            return products.sort((a, b) => a.brandDisplay.localeCompare(b.brandDisplay));
        case 'popularity':
...
function createProductCard(product) {
    ...
    col.className = currentView === 'list' ? 'col-12' : 'col-xl-4 col-lg-4 col-md-6 col-sm-6';
            ...
            <div class="card">
                ...
                <div class="product-info">
                    ...
                    <div class="product-actions">
                        ...
                        <button class="btn btn-quickview" onclick="quickView(${product.id})" title="Xem nhanh">
                            <i class="fas fa-eye"></i>
                        </button>
                        <button class="btn btn-favorite" onclick="toggleFavorite(${product.id})" title="Yêu thích">
...
function updateUI() {
    ...
    // Update counters
    if (totalProductsSpan) totalProductsSpan.textContent = productsDatabase.length;
...
function updateActiveFilters() {
    ...
    const hasActiveFilters = activeFilters.gender.length > 0 || 
    ...
    if (hasActiveFilters) {
        ...
        if (activeFilters.search.trim() !== '') {
            const searchTag = createFilterTag('Tìm kiếm', `"${activeFilters.search}"`, () => {
        ...
        activeFilters.gender.forEach(gender => {
            const displayName = gender === 'men' ? 'Nam' : gender === 'women' ? 'Nữ' : 'Unisex';
            const tag = createFilterTag('Giới tính', displayName, () => removeActiveFilter('gender', gender));
        ...
        activeFilters.brand.forEach(brand => {
            ...
            const tag = createFilterTag('Thương hiệu', displayName, () => removeActiveFilter('brand', brand));
            activeFilterTags.appendChild(tag);
        });
        
        // Add season filter tags
        activeFilters.season.forEach(season => {
            const displayName = season === 'spring' ? 'Xuân' : 
                              season === 'summer' ? 'Hạ' : 
                              season === 'autumn' ? 'Thu' : 'Đông';
            const tag = createFilterTag('Mùa', displayName, () => removeActiveFilter('season', season));
            activeFilterTags.appendChild(tag);
        });
    } else {
